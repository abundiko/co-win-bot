...
client.on("ready", async () => {
  ...
  setInterval(async () => {
    if (sessionHasUpdated) {
      ...
      if (await updateSession(session)) console.log("session updated", session);
    }
  }, 5000);
});
...
client.on("message", async message => {
  if (message.body && hasLoaded) {
    ...
    try {
      if (session[sender]["stage"] && session[sender]["stage"] != "final")
        sessionHasUpdated = true;
    } catch (e) {}
    if (!session[sender]) {
      ...
      switch (session[sender]["stage"]) {
        case "fullname":
          ...
          await client.sendMessage(sender, welcome.fullnameConfirm(fullname));
